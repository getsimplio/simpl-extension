...
| Biometric unlock credential | WebAuthn-PRF platform authenticator (Touch ID / Windows Hello), opt-in; no native host | Unlock convenience | Local device authenticator only (not a remote party) | Until user disables biometric unlock | Settings → disable | Not collected / not transmitted (local device) |
...
| Token / market prices & charts | Fetched from the Simpl API gateway (`api.getsimpl.io`); CoinGecko is an upstream source **behind** the gateway, not contacted directly | Show fiat values & charts | Request sends **token contract address + chainId + range** (public identifiers), **not** your wallet address | Not retained by the wallet beyond cache | n/a | Required for functionality |
...
- Blockchain RPC nodes: PublicNode (ETH/Base/BSC/Sepolia/Solana), drpc (Solana),
  Solana public RPC, TronGrid, Bitcoin Esplora (blockstream.info). They observe
  the queried address and the client IP — inherent to any wallet.
- `api.getsimpl.io` — first-party gateway for market data, TON RPC, Solana
  portfolio, and the swap/bridge proxy.
- `api.0x.org` — swap quotes fallback when the Simpl swap proxy is not configured.
- WalletConnect / Reown relay + verify/explorer APIs — only when the user
  initiates a WalletConnect session.
- IPFS / Arweave gateways — Solana off-chain token metadata only.

The exact host allowlist (matching `host_permissions`) is in
